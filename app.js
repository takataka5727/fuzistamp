(function () {
  ...
  var STORAGE_KEY = 'fujifilm-camera-stamp-rally-v3';
  ...
  async function initLine() {
    var liffId = config.line && config.line.liffId;
  ...
  function renderWelcome() {
    return '<main class="screen welcome-screen">' + brandHeader({ transparent: true }) +
      ...
      '<div class="welcome-hero__stamp" aria-hidden="true">' + icons.camera + '<b>' + config.booths.length + '</b><span>STAMPS</span></div>' +
      '<p class="welcome-hero__caption">' + E(config.booths[0].name) + ' / ' + E(config.booths[0].zone) + '</p></section>' +
  ...
  function cameraStamp(booth, size) {
    return '<span class="camera-stamp camera-stamp--' + size + ' camera-stamp--' + E(booth.stampStyle || 'round') + '" style="--stamp-color:' + E(booth.stampColor) +
  ...
  function renderReward() {
    ...
    return '<main class="screen reward-screen' + (state.exchanged ? ' is-exchanged' : '') + '">' +
      ...
      '<div class="reward-ticket__stamp">' + icons.camera + '<span>' + config.booths.length + ' / ' + config.booths.length + '</span></div><div><p>' +
